var Safras = function () {
	...
	var initDelete = function () {
		$('#table-safras').on('click', 'a.btn-remove', function(ev){
			ev.preventDefault();
			param1 = $(this).data("id");
			bootbox.confirm({
				title: "Confirmação",
				...
				callback: function(result) {
					if (result) {
						...
						$.post(baseUrl+"Safras/excluir/"+param1,function(data){

							if (data.status == "ok")
								$("#table-safras").DataTable().ajax.reload();
							...
							App.alert({
								...
								place: "prepend", // append or prepent in container 
								type: "danger",  // alert's type
								message: "Ocorreu um erro ao fazer a exclusão do usuário. "+errorThrown,  // alert's message
								close: true, // make alert closable
								reset: true, // close all previouse alerts first
								focus: true, // auto scroll to the alert after shown
								closeInSeconds: 10, // auto close after defined seconds
	...
	var handleRecords = function () {
		...
		grid.init({
			...
			dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
				...
				"aoColumns": [{}, {}, {"sClass": "text-center"}, {"sClass": "text-center"}, {'bSortable' : false, "sClass": "text-center"}],
				...
				"ajax": {
					"url": baseUrl+"Safras/index", // ajax source
				},
				"order": [
					[1, "asc"]
