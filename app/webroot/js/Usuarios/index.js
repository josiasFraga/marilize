var Index = function () {
	...
	var initDelete = function () {
		$('#table-usuarios').on('click', 'a.btn-remove', function(ev){
			ev.preventDefault();
			param1 = $(this).data("id");
			bootbox.confirm({
				title: "Confirmação",
				...
				callback: function(result) {
					if (result) {
						App.blockUI({
							target: '#table-usuarios',
							boxed: true,
							message: 'Excluindo, aguarde...'
						});
						$.post(baseUrl+"Usuarios/excluir/"+param1,function(data){
							if (data.status == "ok")
	...
	var handleRecords = function () {
		...
		grid.init({
			...
			"aoColumns": [
				...
				"ajax": {
					"url": baseUrl+"Usuarios/index", // ajax source
				},
				"order": [
					[1, "asc"]
