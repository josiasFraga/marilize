var Index = function () {
	...
	var initDelete = function () {
		$('#table-pessoas').on('click', 'a.btn-remove', function(ev){
			ev.preventDefault();
			param1 = $(this).data("id");
			bootbox.confirm({
				title: "Confirmação",
				...
				callback: function(result) {
					if (result) {
						App.blockUI({
							target: '#table-pessoas',
							boxed: true,
							message: 'Excluindo, aguarde...'
						});
						$.post(baseUrl+"Pessoas/excluir/"+param1,function(data){
							if (data.status == "ok")
								$("#table-pessoas").DataTable().ajax.reload();
	...
	var handleRecords = function () {
		...
		grid.init({
			...
			"aoColumns": [
				...
				"ajax": {
					"url": baseUrl+"Pessoas/index/" + window.tipo_pessoa, // ajax source
				},
				"order": [
					[1, "asc"]
