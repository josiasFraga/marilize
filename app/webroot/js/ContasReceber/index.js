var Index = function () {
	...
	var initDelete = function () {
		$('#table-receberpagamentos').on('click', 'a.btn-remove', function(ev){
			ev.preventDefault();
			param1 = $(this).data("id");
			bootbox.confirm({
				title: "Confirmação",
				...
				callback: function(result) {
					if (result) {
						App.blockUI({
							target: '#table-receberpagamentos',
							boxed: true,
							message: 'Excluindo, aguarde...'
						});
						$.post(baseUrl +"ContasReceber/excluir/"+param1,function(data){
							if (data.status == "ok")
								$("#table-receberpagamentos").DataTable().ajax.reload();
							...
							App.unblockUI('#table-receberpagamentos');
						}).fail(function(jqXHR, textStatus, errorThrown){
							App.alert({
								container: "div.portlet-body", // alerts parent container(by default placed after the page breadcrumbs)
								place: "prepend", // append or prepent in container
								type: "danger", // alert's type
								message: "Ocorreu um erro ao fazer a exclusão da conta à receber. "+errorThrown, // alert's message
								close: true, // make alert closable
								reset: true, // close all previouse alerts first
								focus: true, // auto scroll to the alert after shown
								closeInSeconds: 10, // auto close after defined seconds
	...
	var handleRecords = function () {
		...
		grid.init({
			...
			"aoColumns": [
				...
				"ajax": {
					"url": baseUrl +"ContasReceber/index", // ajax source
				},
				"order": [
					[2, "asc"]
					// [5, "asc"]
		...
		$("a.tool-action").click(function(ev) {
			...
			if ( action == 4 ) { //excluir
				if ( grid.getSelectedRowsCount() == 0 ) {
					...
					$.post(baseUrl+"ContasReceber/excluirVarious/?"+url_ids,function(data){
						if (data.status == "ok") {
							App.alert({
								container: "div.portlet-body", // alerts parent container(by default placed after the page breadcrumbs)
								place: "prepend", // append or prepent in container
				...
				if ( grid.getSelectedRowsCount() == 0 ) {
					...
					$("form#pagarvarious #adicionar").click(function(e){
						...
						if (data_pago == "" || forma_pago == "") {
							...
							$.post(baseUrl+"ContasReceber/pagarVarious/?"+url_ids, function(data) {
								if (data.status == "ok") {
									$('#alert-pagar').empty().unbind();
									App.alert({
	...
	var initPago = function () {
		$('#table-receberpagamentos').on('click', 'a.btn-pagar', function (ev) {
			ev.preventDefault();
			param1 = $(this).data("id");
			bootbox.confirm({
				title: "Confirmação",
				...
				callback: function (result) {
					if (result) {
						...
						$.post(baseUrl + "ContasReceber/pagamento/" + param1, function (data) {
							if (data.status == "ok") {
								// $("#table-receberpagamentos .alert-success span.message").html(data.msg);
	...
	var handleSample = function () {
		$('#table-receberpagamentos').on('click', 'a.btn-popup', function(ev) {
			ev.preventDefault();
	...
	var handleValidationAlterar = function() {
		$("form#alterar-receber button[type=submit]").click(function(e){
            e.preventDefault();
			$('form#alterar-receber').submit();
        });
        // for more info visit the official plugin documentation:
        // http://docs.jquery.com/Plugins/Validation

		var form = $('form#alterar-receber');
        ...
        form.validate({
            ...
            submitHandler: function (form) {
				...
				$("form#alterar-receber button[type=submit]").html('<i class="fa fa-spinner fa-spin"></i> Cadastrando, aguarde...').attr('disabled',true);
                $.ajax({
                    type: "POST",
                    data: formdata,
                    ...
                    if (data.status == "ok"){
						$("form#alterar-receber .alert-success span.message").html(data.msg);
                        success.show();
                        error.hide();
                        warning.hide();
                        // form.reset();
						...
						$("form#alterar-receber .alert-warning span.message").html(data.msg);
                        warning.show();
                        success.hide();
                        error.hide();
                    }else if(data.status == "erro"){
						$("form#alterar-receber .alert-danger span.message").html(data.msg);
                        warning.hide();
                        success.hide();
                        error.show();
					}
					...
					$("form#alterar-receber button[type=submit]").html('<i class="fa fa-check"></i> Adicionar').removeAttr('disabled');
					App.scrollTo($("form#alterar-receber"), -200);
                }).fail(function(jqXHR, textStatus, errorThrown){
                    success.hide();
                    warning.hide();
					$("form#alterar-receber .alert-danger span.message").html("Ocorreu um erro ao alterar a conta à receber. ("+textStatus+" - "+errorThrown+")")
                    error.show();
					$("form#alterar-receber button[type=submit]").html('<i class="fa fa-check"></i> Adicionar').removeAttr('disabled');
                    App.scrollTo(error, -200);
                });
	...
	var initPrint = function() {
		$('a#imprimir_contas').click(function(ev){
			...
			window.open(baseUrl + 'Contas/imprimir/e/?' + params_str, '_blank').focus();
		});
	}
