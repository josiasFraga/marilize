var Adicionar = function () {
    ...
    var handleValidation = function() {
        $("form#adicionar-contar button[type=submit]").click(function(e){
            e.preventDefault();
            $('form#adicionar-contar').submit();
        });
        // for more info visit the official plugin documentation:
        // http://docs.jquery.com/Plugins/Validation

        var form = $('form#adicionar-contar');
        ...
        form.validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    data: formdata,
                    url: baseUrl +'ContasReceber/adicionar',
                    async: true,
                    cache: false,
                    processData: false,
                    contentType: false
                    ...
                    if (data.status == "ok"){
                        $("form#adicionar-contar .alert-success span.message").html(data.msg);
                        success.show();
                        error.hide();
                        warning.hide();
                        $('#outra_conta').show();
                        ...
                        $("form#adicionar-contar .alert-danger span.message").html(data.msg);
                        warning.hide();
                        success.hide();
                        error.show();
                    }
                    $("form#adicionar-contar button[type=submit]").html('<i class="fa fa-check"></i> Salvar').removeAttr('disabled');
                    App.scrollTo($("form#adicionar-contar"), -200);
                }).fail(function(jqXHR, textStatus, errorThrown){
                    success.hide();
                    warning.hide();
                    $("form#adicionar-contar .alert-danger span.message").html("Ocorreu um erro ao adicionar o conta à pagar. ("+textStatus+" - "+errorThrown+")")
                    error.show();
                    $("form#adicionar-contar button[type=submit]").html('<i class="fa fa-check"></i> Salvar').removeAttr('disabled');
                    App.scrollTo(error, -200);
                });
