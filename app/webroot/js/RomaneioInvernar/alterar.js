var Adicionar = function () {
            ...
            submitHandler: function (form) {
                ...
                if (confirma) {
                    $.ajax({
                        type: "POST",
                        data: formdata,
                        url: baseUrl+'RomaneioInvernar/alterar',
                        async: true,
                        cache: false,
                        processData: false,
                        contentType: false
    ...
    var initDeleteEspecie = function () {
		$('#table-especies').on('click', 'a.btn-remove', function(ev){
            ev.preventDefault();
            var este = $(this);
			param1 = $(this).data("id");
			bootbox.confirm({
				...
				callback: function(result) {
					if (result) {
						App.blockUI({
							target: '#table-especies',
							boxed: true,
							message: 'Excluindo, aguarde...'
						});
						$.post(baseUrl+"RomaneioInvernar/excluirEspecie/"+param1,function(data){
							if (data.status == "ok") {
			...
			bootbox.confirm({
				...
				callback: function(result) {
					if (result) {
						...
						$.post(baseUrl+"RomaneioInvernar/excluirArquivo/"+param1,function(data){
							if (data.status == "ok") {
