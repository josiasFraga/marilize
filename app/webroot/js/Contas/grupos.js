var Grupos = function () {
	...
	var initDelete = function () {
		$('#table-grupos').on('click', 'a.btn-remove', function(ev){
			ev.preventDefault();
			param1 = $(this).data("id");
			bootbox.confirm({
				title: "Confirmação",
				...
				callback: function(result) {
					if (result) {
						...
						$.post(baseUrl +"Contas/excluir_grupo/"+param1,function(data){
							if (data.status == "ok")
								$("#table-grupos").DataTable().ajax.reload();
	...
	var handleRecords = function () {
		...
		grid.init({
			...
			"aoColumns": [
				...
				"ajax": {
					"url": baseUrl +"Contas/grupos", // ajax source
				},
				"order": [
					[1, "asc"]
	...
	var initPago = function () {
		$('#table-grupos').on('click', 'a.btn-pagar', function (ev) {
			ev.preventDefault();
			param1 = $(this).data("id");
			bootbox.confirm({
				title: "Confirmação",
				...
				callback: function (result) {
					if (result) {
						...
						$.post(baseUrl + "ContasPagar/pagamento/" + param1, function (data) {
							if (data.status == "ok") {
								// $("#table-grupos .alert-success span.message").html(data.msg);
